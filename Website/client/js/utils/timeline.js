// ==============================
// Timeline Feature â€” Clean & Sorted
// ==============================
...
const MAX_ZOOM_MAP = { Brazil: 4.5, Poland: 4.5, "South Korea": 4.5 };

// Per-country visual tweaks to equalize perceived size
const ZOOM_TWEAK = { Brazil: 0.0, Poland: 0.25, "South Korea": 1.10 }; // was 0.55
const PAD_TWEAK  = { Brazil: 0,   Poland: -4,   "South Korea": -24 };  // was -8

// Known country name â†’ ISO3
  ...
  COUNTRIES.forEach(name => {
    ...
    if (layer) {
      ...
      layer.eachLayer(shape => {
        shape.unbindTooltip();
        const txt = `${name} â€¢ ${metric}: ${envVal == null ? "n/a" : envVal.toFixed(2)}`;
